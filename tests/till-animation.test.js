...
// พจนานุกรมใช้ไฟล์เดียวกันทั้งสองรุ่น แต่รุ่นเก็บข้อมูลในเครื่องไม่มีหน้าออเดอร์ออนไลน์
// คิว QR และบางส่วนของสำรองข้อมูล คำพวกนี้จึงมีอยู่จริงในอีกรุ่นหนึ่ง ไม่ใช่กุญแจที่พิมพ์ผิด
const ONLY_IN_OTHER_BUILD = [
  'QR ชำระเงิน', 'QR โต๊ะ / จุดรับสินค้า', 'กรุณาระบุชื่อโต๊ะหรือจุดรับสินค้าก่อน', 'กรุณาระบุเหตุผลก่อนปฏิเสธ',
  ...
  'สั่งอาหารออนไลน์', 'ส่งให้ลูกค้าแล้ว', 'ออเดอร์ออนไลน์รอยืนยัน',
  'อัปโหลดรูป QR พร้อมเพย์/บัญชีร้านครั้งเดียว ลูกค้าที่สั่งออนไลน์ทุกคนจะเห็นรูปนี้ตอนชำระเงิน',
  'อัพโหลดรูป QR ชำระเงินเรียบร้อยแล้ว ลูกค้าจะเห็นรูปนี้ตอนสั่งออนไลน์', 'อัพโหลดรูป QR ไม่สำเร็จ:',
  'เชื่อมต่อไม่สำเร็จ กรุณาลองใหม่', 'เช็คออเดอร์ออนไลน์ไม่สำเร็จ',
  'เซิร์ฟเวอร์ยังไม่ได้เก็บรูปไว้ กรุณาลองใหม่อีกครั้ง', 'เวลาชงต่อ 1 แก้ว (นาที)', 'ใส่ตัวเลขติดลบไม่ได้',
  'ไฟล์รูปนี้เปิดไม่ได้ ลองบันทึกเป็น JPG หรือ PNG แล้วอัพโหลดใหม่', 'ไม่มีสินค้าในหมวดนี้'];
...
test('the dictionary translates into Burmese, not back into Thai', () => {
  const dict = langDict();
  // ฿ (U+0E3F) อยู่ในช่วงตัวอักษรไทยก็จริง แต่เป็นสัญลักษณ์เงินบาท ต้องคงไว้
  // และบางข้อความสั่งให้พิมพ์คำไทยตามตัว คำนั้นจึงต้องเป็นไทยอยู่
  const MUST_KEEP_THAI = ['พิมพ์คำว่า "กู้คืน" เพื่อยืนยันอีกครั้ง:'];
  const bad = Object.entries(dict)
    .filter(([k]) => !MUST_KEEP_THAI.includes(k))
    .filter(([, v]) => /[\u0E00-\u0E3E\u0E40-\u0E7F]/.test(v));
